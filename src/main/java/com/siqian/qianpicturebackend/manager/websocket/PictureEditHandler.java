...
import com.siqian.qianpicturebackend.manager.websocket.disruptor.PictureEditEventProducer;
import com.siqian.qianpicturebackend.manager.websocket.model.PictureEditActionEnum;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    /**
     * 每张图片的编辑状态，key:图片id，value:正在编辑用户id
     */
    private final Map<Long, Long> pictureEditingUsers = new ConcurrentHashMap<>();
    /**
     * 保存所有连接的会话，key:图片id，value:用户会话集合
     */
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        // 获取消息内容，将Json 转换为PictureEditRequestMessage
        ...
        User user = (User) session.getAttributes().get("user");
        Long pictureId = (Long) session.getAttributes().get("pictureId");
        // 根据消息类型处理消息（生产消息到Disruptor环形队列中）
     ...
     * @param pictureId
     */
    public void handelEnterEditMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        // 没有用户正在编辑该图片，才能进入编辑
     ...
     * @param pictureId
     */
    public void handelEditActionMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        // 正在编辑的用户
     ...
     * @param pictureId
     */
    public void handelExitEditMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        // 正在编辑的用户
        Long editingUserId = pictureEditingUsers.get(pictureId);
        // 确认是当前的编辑者
        if (editingUserId != null && editingUserId.equals(user.getId())){
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        Long pictureId = (Long) session.getAttributes().get("pictureId");
        // 移除当前用户的编辑状态
        handelExitEditMessage(null, session, user, pictureId);
        // 删除会话
    ...
    /**
     * 将消息广播给该图片的所有用户(支持排除广播某个用户)
     * @param pictureId
     * @param pictureEditResponseMessage
     * @param excludeSession 不需要广播的用户
     */
